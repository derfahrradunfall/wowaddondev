...
-- GEFIXT: Die alte Implementierung hat pro Liste EINMALIG entschieden,
-- ob es sich um das alte Array-Format ({index -> itemID}) handelt, und
-- ist dabei über ALLE Einträge der Liste gelaufen (pairs() ohne
-- garantierte Reihenfolge). Sobald dabei irgendwo ein einzelner alter
-- Array-Eintrag (Zahl -> Zahl) gefunden wurde, wurden ALLE anderen,
-- bereits korrekten Einträge im neuen Format (itemID -> true) beim
-- Neuaufbau der Tabelle stillschweigend VERWORFEN, weil dort nur nach
-- "type(v) == number" gefiltert wurde. Dadurch konnte ein grünes,
-- gewhitelistetes Item bei einem UI-Reload/Login plötzlich wieder aus
-- der Whitelist verschwinden und danach als graues Item gelöscht werden.
--
-- Der Fix behandelt jeden Eintrag EINZELN, sodass kein gültiger
-- Whitelist-/Blacklist-Eintrag mehr verloren gehen kann, egal in
-- welchem Format er vorliegt.
local function MigrateListToKeyValue(list)
    if not list then return {} end
    local migrated = {}
    for k, v in pairs(list) do
        if type(k) == "number" and v == true then
            -- Bereits im aktuellen Format: [itemID] = true
            migrated[k] = true
        elseif type(v) == "number" then
            -- Altes Format: [index] = itemID
            migrated[v] = true
        elseif type(k) == "number" then
            -- Unbekannter/Fremd-Wert, aber gültiger numerischer Key (itemID) -> übernehmen statt verwerfen
            migrated[k] = true
        end
    end
    return migrated
...
local function ProcessSpaceProtection(isManualClick)
    ...
    if neededSlots > 0 then
        ...
        for i = 1, loops do
            ...
            -- Letzter Sicherheits-Check unmittelbar vor dem Löschen: Whitelist hat IMMER Vorrang.
