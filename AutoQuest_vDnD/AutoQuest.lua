...
eventFrame:SetScript("OnEvent", function(self, event)
	...
	if event == "PLAYER_ENTERING_WORLD" then
		...
		-- NOTE: da AutoQuestSave jetzt account-weit (SavedVariables) gespeichert wird,
		-- werden diese Defaults nur beim allerersten Login auf dem Account gesetzt.
		-- Danach gelten die gespeicherten Werte fuer ALLE Charaktere des Accounts gemeinsam.
		if AutoQuestSave              == nil then AutoQuestSave              = {}    end
		if AutoQuestSave.autoAccept   == nil then AutoQuestSave.autoAccept   = false end
		if AutoQuestSave.autoDaily    == nil then AutoQuestSave.autoDaily    = false end -- #2: Standard jetzt AUS (vorher true)
		if AutoQuestSave.autoFate     == nil then AutoQuestSave.autoFate     = false end -- #2: Standard jetzt AUS (vorher true)
		if AutoQuestSave.autoRepeat   == nil then AutoQuestSave.autoRepeat   = false end -- #2: Standard jetzt AUS (vorher true)
		if AutoQuestSave.autoComplete == nil then AutoQuestSave.autoComplete = true  end
		if AutoQuestSave.autoHR       == nil then AutoQuestSave.autoHR       = false end -- #2: explizit auf AUS gesetzt (vorher nil/undefiniert)
